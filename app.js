...
async function orderNow() {
    ...
    const newOrder = {
        studentName: "student1",  // or store username in localStorage if you wish
        items: [...appState.cart],
        location: selectedLocation,
        total: appState.cart.reduce((sum, item) => sum + (item.price * item.quantity), 0),
        estimatedDelivery: Math.floor(Math.random() * 25 + 20),
        status: "confirmed"
    };
    ...
    try {
        ...
        console.log("Order placed and saved:", savedOrder);

        appState.cart = [];
        updateCartDisplay();
        toggleCart();
        alert(`Order placed to ${selectedLocation}!`);
...
async function fetchAndRenderOrders() {
    ...
    try {
        ...
        appState.studentOrders = orders;
        renderStudentOrders();
    } catch (err) {
        console.error(err);
        alert("Could not load your orders. Please check your network.");
    }
}
...
document.getElementById("signup-form").addEventListener("submit", async (e) => {
    ...
    try {
        ...
        if (data.message === "Registered successfully") {
            document.getElementById("signup-status").innerText = "✅ Registered successfully, please login!";
            toggleAuth("login");
        } else {
            document.getElementById("signup-status").innerText = "❌ Signup failed. Try again.";
        }

    } catch (err) {
        console.error(err);
        document.getElementById("signup-status").innerText = "❌ Error connecting to server.";
    }
});
